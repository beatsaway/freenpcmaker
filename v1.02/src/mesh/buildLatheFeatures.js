...
function noseRecipe(style, sc, width = 1) {
  const w = clamp(width, 0.5, 2.0);
  // Defaults — a bit larger than v1.01 (closer to v1.0 presence, still mild)
  let bridgeTopY = 0.016 * sc;
  let bridgeBotY = -0.002 * sc;
  let bridgeZ0 = 0.001 * sc;
  let bridgeZ1 = 0.019 * sc;
  let bridgeR = 0.0052 * sc;
  let tipY = -0.007 * sc;
  let tipZ = 0.024 * sc;
  let tipRx = 0.012 * sc;
  let tipRy = 0.009 * sc;
  ...
  // v1.02: grow past mild v1.01 — wider / a bit deeper, still no beak caricature
  const GROW = 1.16;
  bridgeR *= GROW;
  bridgeZ1 *= 1.1;
  tipRx *= GROW * 1.06;
  ...
  // Cap protrusion — wider/deeper than v1.01, still shy of caricature beaks
  tipZ = Math.min(tipZ, 0.034 * sc);
  bridgeZ1 = Math.min(bridgeZ1, tipZ * 0.95);

  // Width scales lateral radii (bridge + tip X) — stronger spread for variation
  bridgeR *= mix(0.7, 1.5, (w - 0.5) / 1.5);
  tipRx *= mix(0.82, 1.2, (w - 0.5) / 1.5) * w;
  tipRz *= mix(0.88, 1.18, (w - 0.5) / 1.5);
...
export function noseTipLocal(opts = {}) {
  ...
  const sc = clamp(opts.scale ?? 1, 0.35, 2.0);
  const width = clamp(opts.width ?? 1, 0.5, 2.0);
