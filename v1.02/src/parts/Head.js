...
import { buildSmoothEar } from "../mesh/buildSmoothEar.js";
import {
  buildLatheFace,
  jawLayout,
...
export class FaceFeatures {
  ...
  static addNose(g, cfg, headY, hd, headMesh, faceOpts) {
    ...
    const scale = (cfg.nose?.scale ?? 1) * faceFeatureScale(hw, hh);
  ...
  /** SDF ears seated on temple surface (v1.0 approach). */
  static addEars(g, cfg, headY, hw, hh, headMesh, faceOpts) {
    ...
    const skin = skinMaterial(cfg.skinTone);
    const y = headY + (style === "point" || style === "elf" ? 0.01 : 0);
    const z = 0;
    const SINK = -0.01;
