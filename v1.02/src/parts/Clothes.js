...
/** Trunk front Z at Y — for polo/jacket accent seating on the SDF shell. */
function clothesFrontZ(st, y = null) {
  ...
  // SDF tops pad ~0.018 outside the body
  const pad = 0.02;
  const rChest = Math.max(chestRX, chestRZ) + pad;
  const rNeck = (L.neckR ?? 0.034) + pad * 0.6;
  ...
  let r = rChest;
  if (yy >= yTop) r = rNeck;
  else if (yy >= yUpper) r = mix(rChest * 0.92, rNeck, (yy - yUpper) / Math.max(1e-6, yTop - yUpper));
  return joinZ + r * joinScaleZ;
}

function accentZ(st, y, depth, embed = 0.002) {
...
export class Clothes {
  ...
  static buildBottom(cfg) {
    ...
    if (style === "mini-shorts") {
      ...
      yTop = Math.min(st.hip.top - 0.015, (st.waistY ?? st.hip.top) - 0.02);
  ...
  static addButtons(g, st, count, { yStart, yEnd, size = 0.016, color = 0x1a1a1a } = {}) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const m = roundBoxMesh(btnSize, btnSize, depth, btn, 0, y, accentZ(st, y, depth, 0.002), btnSize * 0.28);
  ...
  static buildTop(cfg) {
    ...
    if (style === "polo") {
      ...
      const collarM = roundBoxMesh(
        ...
        0,
        collarY,
        accentZ(st, collarY, collarD, 0.002),
        0.005
      );
